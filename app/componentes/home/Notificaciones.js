...
const Notificaciones = ({ route }) => {
	...
	useEffect(async () => {
		...
		if (!usuarioInfo.expo_tokens || usuarioInfo.length < 1) {
			...
			const rta_anadirToken = await axios.post(
				constantes.API + "/consultas/anadirTokenDispositivoExpo",
				{ token: token }
			); 
			console.log("rta_anadirToken", rta_anadirToken);
		} else {
			...
			{loading ? (
				...
				<>
					{ notificaciones !== null ?
						...
						numColumns={1}
						/> :
						<Text style={{...styles.text, marginTop: 20}}>Se produjo un error o no se encuentran notificaciones.</Text>
					}
				</>
			)}
		</View>
